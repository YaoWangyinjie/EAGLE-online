...
def generate_answer(
    ...
    """Run one generation pass and return the assistant text (decoded, stripped)."""
    # apply_chat_template adds the generation prompt automatically
    ...
    with open(args.output_file, "w", encoding="utf-8") as fout:
        for q in tqdm(questions, desc="Questions"):
            ...
            for sample_idx in range(args.num_samples_per_question):
                conversation = []  # will hold {"from": ..., "value": ...} dicts

                # ---- Turn 1 ------------------------------------------------
                turn1_q = turns[0]
                messages_t1 = build_messages_turn1(tokenizer, turn1_q)
                turn1_a = generate_answer(
                ...
                for turn_idx in range(1, len(turns)):
                    ...
                    # Build context: all previous human/gpt pairs + new question
                    prior_messages = []
