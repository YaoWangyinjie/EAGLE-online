...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "and --output_file to  sft_overfit/data/{dataset}_sft.jsonl "
    ...
    if args.dataset is not None:
        ...
        # Auto-derive output_file: sft_overfit/data/{dataset}_sft.jsonl
        if not any(a.startswith("--output_file") for a in argv):
            args.output_file = os.path.join(script_dir, "data", f"{args.dataset}_sft.jsonl")
...
def normalize_question(record: dict, idx: int) -> dict:
    ...
    by detecting the source dataset format from the fields present.
    ...
    """
    # mt_bench and any dataset already in the target format
    if "turns" in record:
        return record

    out = dict(record)  # shallow copy, never mutates the original

    # HumanEval: {"task_id": "HumanEval/0", "prompt": "...", ...}
    if "prompt" in record:
        out["question_id"] = record.get("task_id", str(idx))
        out["turns"] = [record["prompt"]]
        return out

    # MATH / competition-math: {"problem": "...", "solution": "...", ...}
    if "problem" in record:
        out["question_id"] = record.get("id", str(idx))
        out["turns"] = [record["problem"]]
        return out

    # Generic fallback: use the first string-valued field as the single question
...
def main():
    ...
    with open(args.output_file, "w", encoding="utf-8") as fout:
        for idx, q in enumerate(tqdm(questions, desc="Questions")):
