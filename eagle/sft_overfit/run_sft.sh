...
# Dataset name — controls where question.jsonl is read from and where all
# outputs (data, checkpoints, eval_results) are written.
# Override via:  DATASET=humaneval bash run_sft_experiment.sh
# or:            bash run_sft_experiment.sh --dataset humaneval
DATASET="${DATASET:-mt_bench}"

# Derived paths — all namespaced under ${DATASET}
QUESTION_FILE="${EAGLE_DATA_DIR}/${DATASET}/question.jsonl"
SFT_DATA_FILE="${SCRIPT_DIR}/data/${DATASET}_sft.jsonl"
CHECKPOINT_DIR="${SCRIPT_DIR}/checkpoints/${DATASET}"
EVAL_DIR="${SCRIPT_DIR}/eval_results/${DATASET}"
...
# Re-derive paths after potential --dataset override
QUESTION_FILE="${EAGLE_DATA_DIR}/${DATASET}/question.jsonl"
SFT_DATA_FILE="${SCRIPT_DIR}/data/${DATASET}_sft.jsonl"
CHECKPOINT_DIR="${SCRIPT_DIR}/checkpoints/${DATASET}"
EVAL_DIR="${SCRIPT_DIR}/eval_results/${DATASET}"
