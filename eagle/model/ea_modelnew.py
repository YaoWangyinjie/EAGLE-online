...
class EaModel(nn.Module):
    ...
    def __init__(
        ...
        self.adaptation_buffer = []      # 存储当前 question 的 step 数据
        self.train_batch_size = 1        # 批次大小
        self.current_batch_count = 0     # 当前累积的 question 数量
        self.only_first_batch = True     # True: 只训练第一个批次; False: 每个批次都训练
        self.first_batch_trained = False # 标记第一个批次是否已完成训练
    ...
    def setup_online_adaptation(
        ...
        self.first_batch_trained = False
    ...
    def eagenerate(
        ...
        should_collect = use_adaptation and self.adapter_optimizer is not None
        if self.only_first_batch and self.first_batch_trained:
            should_collect = False  # 第一批次已训练完，不再收集
        
        collected_tokens = []      # 收集 accepted tokens
        collected_hiddens = []     # 收集对应的 hidden states
        collected_logits = []      # 收集对应的 target logits
        ...
        prefill_hidden_state = hidden_state.clone() if should_collect else None
        ...
        for idx in range(max_length):
            ...
            if should_collect:
                # 获取当前 step 被接受的 tokens（包含树根，共 accept_length + 1 个）
                # candidates[best_candidate, :accept_length+1] 就是被接受的 token 序列
                step_accepted_tokens = candidates[best_candidate, :accept_length + 1]  # [accept_length+1]
                
                # 获取对应的 hidden states
                if idx == 0:
                    # 第一个 step: 树根的 hidden 来自 prefill
                    first_hidden = prefill_hidden_state[:, -1:, :]  # [1, 1, hidden_dim*3]
                else:
                    # 后续 step: 树根的 hidden 是上一轮保存的 last_step_hidden
                    first_hidden = last_step_hidden  # [1, 1, hidden_dim*3]
                
                if accept_length > 0:
                    # 后续被接受 tokens 的 hidden（从 hidden_state_new 中按 retrieve_indices 取）
                    accepted_indices = retrieve_indices[best_candidate, :accept_length]  # [accept_length]
                    rest_hidden = hidden_state_new[:, accepted_indices, :]  # [1, accept_length, hidden_dim*3]
                    step_hidden = torch.cat([first_hidden, rest_hidden], dim=1)  # [1, accept_length+1, hidden_dim*3]
                else:
                    step_hidden = first_hidden  # [1, 1, hidden_dim*3]
                
                # 获取对应的 target logits
                step_logits = logits[best_candidate, :accept_length + 1, :].unsqueeze(0)  # [1, accept_length+1, vocab_size]
                
                # 保存到收集列表
                collected_tokens.append(step_accepted_tokens.cpu())
                collected_hiddens.append(step_hidden.squeeze(0).cpu())  # [accept_length+1, hidden_dim*3]
                collected_logits.append(step_logits.squeeze(0).cpu())   # [accept_length+1, vocab_size]
                
                # 保存最后一个被接受位置的 hidden，用于下一个 step
                # 这是 sample_p 对应位置的 hidden（即 accept_length 位置）
                last_step_hidden = hidden_state_new[:, retrieve_indices[best_candidate, accept_length]:retrieve_indices[best_candidate, accept_length]+1, :]  # [1, 1, hidden_dim*3]
        ...
        if should_collect and len(collected_tokens) > 0:
            # 拼接当前 question 的所有数据
            all_tokens = torch.cat(collected_tokens, dim=0).unsqueeze(0)    # [1, total_len]
            all_hiddens = torch.cat(collected_hiddens, dim=0).unsqueeze(0)  # [1, total_len, hidden_dim*3]
            all_logits = torch.cat(collected_logits, dim=0).unsqueeze(0)    # [1, total_len, vocab_size]
            
            print(f"\n[Online Adaptation] Question completed. Collected {all_tokens.shape[1]} tokens.")
            
            # 添加到 batch buffer
            if not hasattr(self, 'adaptation_buffer'):
                self.adaptation_buffer = []
            ...
            # 检查是否达到 batch_size，触发训练
            if self.current_batch_count >= self.train_batch_size:
                print(f"\n[Online Adaptation] Training on {self.current_batch_count} questions...")
                
                # 拼接 batch 中所有 question 的数据
                batch_tokens = torch.cat([d['tokens'] for d in self.adaptation_buffer], dim=1)
                batch_hiddens = torch.cat([d['hiddens'] for d in self.adaptation_buffer], dim=1)
                batch_logits = torch.cat([d['logits'] for d in self.adaptation_buffer], dim=1)
                
                print(f"  Total tokens in batch: {batch_tokens.shape[1]}")
                
                # 执行训练
                with torch.enable_grad():
                    avg_loss = self._perform_online_adaptation(
                        batch_tokens,
                        batch_hiddens,
                        batch_logits,
                        train_steps=1
                ...
                adaptation_info['losses'].append(avg_loss)
                ...
                if self.only_first_batch:
                    self.first_batch_trained = True
                    print("[Online Adaptation] First batch training completed. No more training.")
