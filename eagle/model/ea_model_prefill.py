...
class EaModel(nn.Module):
        ...
        with torch.enable_grad():
            for step in range(train_steps):
                ...
                with torch.no_grad():
                    ...
                    # 3. 打印信息
                    print(f"\n[Online Adaptation Prefill Step {step+1}/{train_steps}]")
    ...
    def eagenerate(
        ...
        # === 核心修改：在生成前进行 Prefill 训练 ===
        adaptation_loss = 0.0
        if use_adaptation and self.adapter_optimizer is not None:
            # 1. 状态重置：恢复到初始权重 (Per-question reset)
            if self.initial_weights is not None:
                # 重新加载初始权重
                self.ea_layer.load_state_dict(self.initial_weights, strict=False)
                # print("[Online Adaptation] Weights reset to initial state for new question.")
            else:
                # 如果是第一次运行且还没有保存初始权重，setup_online_adaptation 会处理
                pass

            # 2. 获取 Prefill 数据 (Base Model Output)
            # 我们需要 Base Model 对 input_ids 的完整输出：Hidden States 和 Logits
            # 注意：Base Model 在推理时通常是冻结的，这里也不需要计算 Base Model 的梯度
            with torch.no_grad():
            ...
            adaptation_loss = self._perform_online_adaptation(
                all_accepted_tokens=input_ids,
                all_hidden_states=prefill_hidden_states,
                all_target_logits=prefill_logits,
                train_steps=1 # 只训练一遍
            )
            # print(f"[Online Adaptation] Prefill training done. Loss: {adaptation_loss:.4f}")
        ...
        # prefill
        # 注意：这里 initialize_tree 内部会再次调用 Base Model。
        # 虽然我们在上面为了训练已经调了一次，但为了保持代码对复杂 KV Cache 初始化的兼容性，
        # 我们这里允许重复计算一次。
        with torch.no_grad():
        ...
        # 在函数结束前，原则上不需要手动恢复权重，因为下一个 eagenerate 开头会恢复。
        # 但为了安全起见，或者如果用户可能会关闭 adaptation，可以在这里恢复。
        # 考虑到逻辑简洁性，我们依赖开头的恢复逻辑。
        ...
        if use_adaptation:
             print(f"[Online Adaptation] Prefill loss={adaptation_loss:.4f}")
